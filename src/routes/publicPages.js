...
router.get("/goc-hoc-tap", (req, res) => {
  ...
  const publicResources = getPublishedResourceEntries()
    ...
    .slice(0, 6)
    .map((item, index) => ({
      label: item.title || `Tài liệu ${String(index + 1).padStart(2, "0")}`,
  ...
  const studyHub = {
    ...
    helperText: isStudent
      ...
      : "Một số thư viện luyện sau sẽ mở tốt nhất sau khi đăng nhập tài khoản học viên.",
    groups: [
      {
      ...
      ...(publicResources.length
        ? [
            {
              heading: "Tài liệu mới từ trung tâm",
              layout: "single",
              cards: [
                {
                  tone: "lavender",
